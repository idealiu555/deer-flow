...
import src.scheduler.service as scheduler_service_module
from src.config.scheduler_config import SchedulerConfig
from src.scheduler.service import SchedulerService, _normalize_run_params_for_langgraph
...
async def test_run_claimed_fails_when_channel_delivery_unavailable(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(scheduler_service_module, "_extract_response_text", lambda _result: "ok")
    monkeypatch.setattr(scheduler_service_module, "_extract_artifacts", lambda _result: [])
    monkeypatch.setattr("src.channels.service.get_channel_service", lambda: None)
...
async def test_deliver_outbound_uses_topic_id_as_thread_ts(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("src.channels.service.get_channel_service", lambda: _ChannelService())
